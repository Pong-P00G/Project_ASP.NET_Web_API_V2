...
<template>
  <div class="min-h-screen bg-gray-800 relative" style="font-family: 'Roboto', sans-serif;">
    ...
    <div class="flex relative z-10 min-h-screen">
      ...
      <!-- Sidebar -->
      <aside :class="['fixed top-0 left-0 bottom-0 z-50 flex flex-col sidebar-glass transition-all duration-500 ease-out',
        ...
        <div
          class="absolute inset-0 rounded-3xl p-1px bg-linear-to-b from-white/20 via-transparent to-white/5 pointer-events-none">
        </div>

        <!-- Logo -->
        <div class="h-24 flex items-center justify-between px-6 border-b border-white/10 shrink-0 relative">
          <div class="flex items-center gap-4">
            <div class="logo-icon w-12 h-12 flex items-center justify-center rounded-2xl shrink-0 relative overflow-hidden"
              :class="['animate-pulse-slow', !isSidebarOpen && 'hidden']">
              <div class="absolute inset-0 bg-linear-to-br from-orange-500 to-pink-500"></div>
              <div class="absolute inset-0 bg-linear-to-br from-orange-400 to-pink-600 opacity-0 hover:opacity-100 transition-opacity duration-300"></div>
              <Sparkles class="w-6 h-6 relative z-10 text-white animate-pulse-slow"/>
            </div>
          ...
          <button @click="toggleSidebar"
            ...
            <ChevronLeft :class="['w-4 h-4 transition-transform duration-500', !isSidebarOpen && 'rotate-180 animate-pulse-slow']" />
          </button>
        ...
        <nav class="flex-1 overflow-y-auto py-8 px-3">
          ...
          <router-link v-for="item in navigation" :key="item.name" :to="item.to" @click="closeMobileSidebar" :class="[
            'nav-item flex items-center gap-4 p-3 mb-2 rounded-2xl transition-all duration-300 relative group',
            ...
            <div v-if="route.path.startsWith(item.to)"
              class="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-linear-to-b from-orange-400 to-pink-500 rounded-r-full shadow-glow">
            </div>
          ...
          <div class="flex items-center gap-3 p-2 rounded-2xl transition-all duration-300 group cursor-pointer">
            <div class="relative">
              <div @click="toggleSidebar"
                class="w-11 h-11 flex items-center justify-center bg-linear-to-br from-orange-500 to-pink-500 text-white text-sm font-bold rounded-xl cursor-pointer transition-all duration-300 group-hover:scale-110 group-hover:shadow-lg group-hover:shadow-orange-500/30 shrink-0">
                {{ userInitials }}
              </div>
              <!-- Online Status -->
              <div
                class="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-emerald-400 rounded-full border-2 border-neutral-900 shadow-lg shadow-emerald-400/50">
              </div>
            </div>
            <Transition name="slide-fade">
              <div v-if="isSidebarOpen" class="flex-1 min-w-0">
                ...
                <button @click="handleLogout"
                  class="inline-flex items-center gap-1.5 text-xs font-semibold text-orange-400 hover:text-orange-300 transition-colors mt-0.5">
...
.sidebar-glass {
  background: linear-linear(180deg, rgba(15, 15, 15, 0.98) 0%, rgba(10, 10, 10, 0.95) 100%);
...
/* Glow shadow for active indicator */
.shadow-glow {
  box-shadow: 0 0 20px rgba(249, 115, 22, 0.6), 0 0 40px rgba(236, 72, 153, 0.3);
}
